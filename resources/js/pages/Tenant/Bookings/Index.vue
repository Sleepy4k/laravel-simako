...
<template>
    ...
    <DashboardLayout>
        <div>
            <div class="mb-6">
                <h1 class="text-xl font-black text-slate-900">Booking Masuk</h1>
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-slate-100">
                            <tr v-for="booking in props.bookings.data" :key="booking.id" class="hover:bg-slate-50/50 transition-colors">
                                <td class="px-5 py-4">
                                    <p class="font-semibold text-slate-900">{{ booking.user?.userProfile?.name ?? booking.user?.email ?? '-' }}</p>
                                    <p class="text-xs text-slate-400 mt-0.5">{{ booking.user?.phone ?? '' }}</p>
                                </td>
                                <td class="px-5 py-4">
            ...
            <div v-else class="bg-white border border-slate-200 rounded-2xl p-16 text-center shadow-sm">
                <p class="text-sm font-semibold text-slate-700">Belum ada permintaan booking</p>
            </div>
