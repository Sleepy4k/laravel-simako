...
<template>
    <DashboardLayout>
        <div>
            <h1 class="text-xl font-bold text-(--color-text-primary) mb-6">Semua Booking</h1>
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-(--color-border)">
                        <tr v-for="booking in props.bookings.data" :key="booking.id">
                            <td class="px-4 py-3 text-(--color-text-primary)">{{ booking.user?.userProfile?.name ?? booking.user?.email ?? '-' }}</td>
                            <td class="px-4 py-3">
